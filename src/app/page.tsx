...
const founders = [
  {
    ...
    socials: [
      {
        label: "LinkedIn",
        href: "https://www.linkedin.com/in/jarrod-solomon",
      },
      {
        label: "Instagram",
  ...
  },
  {
    ...
    socials: [
      ...
      {
        label: "Instagram",
        href: "https://www.instagram.com/judeashvin",
      },
    ],
  },
];
...
function Carousel({
  ...
  useEffect(() => {
    ...
    // Start at second copy so both directions have room
    requestAnimationFrame(() => { el.scrollLeft = el.scrollWidth / 3; });

    // Seamless loop: jump one copy-width when we drift into the outer copies
    const onScroll = () => {
  ...
  const onPointerUp = () => {
    ...
    const el = trackRef.current;
    if (!el) return;
    // Restore CSS snap — browser snaps to nearest card automatically
    el.style.scrollSnapType = "";
    resumeTimer.current = setTimeout(() => { paused.current = false; }, 2000);
  };

  // Suppress link navigation after a real drag
